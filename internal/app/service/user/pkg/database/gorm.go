...
import (
	...
	"gorm.io/gorm"
	"resume-resolving/internal/app/service/user/config"
	"resume-resolving/internal/pkg"
	database2 "resume-resolving/internal/pkg/database"
)
...
func NewGorm(config *config.Config, database database2.Database) database2.Orm {
	return &Gorm{
		database: database,
		config:   config,
	}
